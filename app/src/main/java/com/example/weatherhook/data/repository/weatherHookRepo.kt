...
class WeatherHookRepo {
    fun loadAllData(): WeatherHookEventList {
        return try {
            ...
            return WeatherHookEventList( mutableListOf(WeatherHookEvent(eventId = 0, active = false, title = "Error", location = Pair(0.2f,0.3f), timeToEvent = 2, relevantDays = "MO", triggers = listOf(Weather(weatherPhenomenon = 2, correspondingIntensity = 2f)).toMutableList())).toMutableList()
            )
        }
    }
